...
const SystemHealth: React.FC = () => {
  ...
  if (loading)
    return (
      <div className='p-8 text-slate-500 font-bold animate-pulse'>
    ...
    <div className='space-y-8 animate-in fade-in duration-700'>
      ...
      <div className='bg-[#0f141b] border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-4 text-slate-300'>
        <h3 className='text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4'>
          Regional Infrastructure Telemetry
        </h3>
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          {health.map((region) => (
            <div
              key={region.region}
              className='p-4 bg-slate-900/50 border border-slate-800 rounded-2xl hover:border-slate-700 transition-all group'
            >
              <div className='flex justify-between items-start mb-2'>
                <strong className='text-white group-hover:text-blue-400 transition-colors'>
                  {region.region}
                </strong>
                <span
              ...
              <div className='flex gap-4 text-xs text-slate-500 font-mono'>
                <span>
                  CPU: <span className='text-slate-300'>{region.cpu}%</span>
                </span>
                <span>
                  MEM: <span className='text-slate-300'>{region.memory}%</span>
                </span>
                <span>
                  UPTIME:{' '}
                  <span className='text-slate-300'>{region.uptime} hrs</span>
                </span>
              </div>
            </div>
          ))}
